...
app.post('/', function (req, res) { 
  var bodyArray = req.body.signed_request.split(".");
    ...
    if (check === consumerSecret) { 
        ...
        res.header('Content-Security-Policy', 'frame-ancestors \'self\' https://eagan-ibm-dev-ed.my.salesforce.com https://eagan-ibm-dev-ed--c.vf.force.com');
        res.render('index', { title: envelope.context.user.userName, req : JSON.stringify(envelope) });
    } else {
        res.header('X-Frame-Options', 'SAMEORIGIN');
        res.header('Content-Security-Policy', 'frame-ancestors \'self\' https://eagan-ibm-dev-ed--c.vf.force.com');
